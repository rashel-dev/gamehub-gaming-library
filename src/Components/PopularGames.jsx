...
const PopularGames = ({ gamesData }) => {
    const popularGames = gamesData.sort((a, b) => parseFloat(b.ratings) - parseFloat(a.ratings)).slice(0, 6);

    return (
        <motion.section initial={{ opacity: 0, x: -100 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} viewport={{ once: false }} className="py-16 bg-base-200">
            <div className="container mx-auto px-4">
                <div className="text-center mb-12">
                    <h2 className="text-3xl md:text-4xl font-bold mb-4">Popular Games</h2>
                    <p className="text-gray-600 max-w-2xl mx-auto">Discover the highest-rated games that players love the most</p>
                </div>
                        ...
                        <Link key={game.id} to={`/game/${game.id}`} className="group">
                            <div className="card bg-base-100 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2">
                                ...
                                <div className="card-body p-4">
                                    ...
                                    <h3 className="card-title text-lg font-bold mb-2 group-hover:text-primary transition-colors">{game.title}</h3>
